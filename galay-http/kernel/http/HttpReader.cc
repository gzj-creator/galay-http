...
namespace galay::http 
{
    ...
    AsyncResult<std::expected<void, HttpError>> HttpReader::getChunkData(const std::function<void(std::string)> &callback, std::chrono::milliseconds timeout)
    {
    ...
    Coroutine<nil> HttpReader::readRequestHeader(AsyncWaiter<HttpRequestHeader, HttpError>& waiter, std::chrono::milliseconds timeout)
    {
        ...
        while(recv_size <= m_params.max_header_size) {
            ...
            if (recv_size >= 4 && std::strncmp(m_buffer.data(), "PRI ", 4) == 0) {
                HTTP_LOG_INFO("[HttpReader] Detected HTTP/2 PRI preface");
    ...
    Coroutine<nil> HttpReader::readRequest(std::shared_ptr<AsyncWaiter<HttpRequest, HttpError>> waiter, std::chrono::milliseconds timeout)
    {
        ...
        SERVER_REQUEST_LOG(header_res.value().method(), header_res.value().uri());
