                                                ...
                                                HttpsConnection& conn, HttpsParams params)
    {
        ...
        try {
            ...
            if (settings.use_sendfile) {
                ...
                if (file_fd < 0) {
                    HTTPS_LOG_DEBUG("[HttpsRouter] Failed to open file for sendfile: {}", strerror(errno));
                    if (!conn.isClosed()) {
                        co_await conn.close();
                    }
                    co_return nil();
                ...
                HTTPS_LOG_INFO(
                    "[HttpsRouter] ========== Sendfile Start: {} bytes ==========",
                    bytes_to_send
                );
                ...
                while (total_sent < bytes_to_send) {
                    ...
                    if (conn.isClosed()) {
                        HTTPS_LOG_WARN(
                            "[HttpsRouter] Connection closed during sendfile at {}/{} bytes (iteration: {})",
                    ...
                    HTTPS_LOG_INFO(
                        "[HttpsRouter] [Iter {}] Before sendfile: sent={}/{} ({:.1f}%), offset={}, chunk_size={}",
                    ...
                    if (!sendfile_res) {
                        HTTPS_LOG_ERROR(
                            "[HttpsRouter] [Iter {}] Sendfile failed at {}/{} bytes: {}",
                    ...
                    HTTPS_LOG_INFO(
                        "[HttpsRouter] [Iter {}] Sent: {} bytes in {} ms ({:.1f} KB/s), total={}/{} ({:.1f}%), avg_speed={:.1f} KB/s",
                    ...
                    if (bytes_sent < 8192 && remaining >= 8192) {
                        HTTPS_LOG_WARN(
                            "[HttpsRouter] [Iter {}] WARNING: Only sent {} bytes (expected more)",
                ...
                HTTPS_LOG_INFO(
                    "[HttpsRouter] ========== Sendfile Complete: {} bytes in {} ms ({:.1f} KB/s, {} iterations) ==========",
