...
Coroutine runClient(uint16_t port) {
    ...
    if (!connect_result) {
        std::cerr << "[T46] client connect failed: " << connect_result.error().message() << "\n";
        g_client_done = true;
        co_return;
    }
    ...
    if (!upgrade_result) {
        std::cerr << "[T46] client upgrade failed: " << upgrade_result.error().toString() << "\n";
        g_client_done = true;
        co_return;
    }
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (!stream) {
            std::cerr << "[T46] post returned null stream at round " << i << "\n";
            g_client_done = true;
            co_return;
        }
        ...
        if (!response_done) {
            std::cerr << "[T46] waitResponseComplete failed at round " << i << "\n";
            g_client_done = true;
            co_return;
        }

        if (stream->response().status != 200 || stream->response().body != body) {
            std::cerr << "[T46] unexpected response at round " << i
    ...
    if (!shutdown_result) {
        std::cerr << "[T46] client shutdown failed\n";
        g_client_done = true;
        co_return;
    }
