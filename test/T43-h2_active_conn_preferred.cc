...
Task<void> runClient(uint16_t port) {
    ...
    if (!connect_result) {
        std::cerr << "[T43] client connect failed: " << connect_result.error().message() << "\n";
        g_client_done = true;
        co_return;
    }
    ...
    if (!upgrade_result) {
        std::cerr << "[T43] client upgrade failed: " << upgrade_result.error().toString() << "\n";
        g_client_done = true;
        co_return;
    }
    ...
    if (!stream) {
        std::cerr << "[T43] client post returned null stream\n";
        g_client_done = true;
        co_return;
    }
    ...
    if (!shutdown_result) {
        std::cerr << "[T43] client shutdown failed\n";
        g_client_done = true;
        co_return;
    }
    g_client_done = true;
...
int main() {
    ...
    if (!g_client_done.load(std::memory_order_acquire)) {
        std::cerr << "[T43] client timed out\n";
        return 1;
    }
    if (!g_client_ok.load(std::memory_order_acquire)) {
        std::cerr << "[T43] client did not receive expected response\n";
        return 1;
    }
    if (g_active_handler_calls.load(std::memory_order_acquire) != 1) {
        std::cerr << "[T43] active handler should run exactly once per connection\n";
        return 1;
    }
    if (g_stream_handler_calls.load(std::memory_order_acquire) != 0) {
        std::cerr << "[T43] legacy stream handler must be bypassed when active handler exists\n";
        return 1;
    }
    if (g_active_deliveries.load(std::memory_order_acquire) != 1) {
        std::cerr << "[T43] expected exactly one active stream delivery, got "
    ...
    if (!hasHttp2StreamEvent(seen, Http2StreamEvent::HeadersReady) ||
        ...
        std::cerr << "[T43] active handler did not observe expected request events\n";
        return 1;
    }

    std::cout << "T43-H2ActiveConnPreferred PASS\n";
    return 0;
}
