...
int main() {
    ...
    if (!waiter1->isReady() || !waiter2->isReady()) {
        std::cerr << "[T61] ssl owner loop should notify all queued waiters even when batched send fails\n";
        return 1;
    }

    if (!stream->isFrameQueueClosed()) {
        std::cerr << "[T61] sslServiceLoop send-fail exit should still close stream queues after batched waiter flush\n";
        return 1;
    }

    if (conn.isClosing()) {
        std::cerr << "[T61] mock send failure should not require transport close just to preserve queue cleanup\n";
        return 1;
    }
