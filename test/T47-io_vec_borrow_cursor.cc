...
int main() {
    ...
    if (state.count() != 2) {
        std::cerr << "[T47] expected normalized count=2, got " << state.count() << "\n";
        return 1;
    }

    if (state.data() == nullptr || state.data()[0].iov_base != first.data() || state.data()[0].iov_len != first.size()) {
        std::cerr << "[T47] unexpected first window after normalize\n";
        return 1;
    }

    const size_t advanced = state.advance(5);
    if (advanced != 5) {
        std::cerr << "[T47] expected advance=5, got " << advanced << "\n";
        return 1;
    }

    if (state.count() != 1) {
        std::cerr << "[T47] expected remaining count=1, got " << state.count() << "\n";
        return 1;
    }

    if (state.data()[0].iov_base != (second.data() + 1) || state.data()[0].iov_len != 2) {
        std::cerr << "[T47] unexpected current window after advance\n";
        return 1;
    }
    ...
    if (window.size() != 1 || window[0].iov_base != (second.data() + 1) || window[0].iov_len != 2) {
        std::cerr << "[T47] unexpected exported window after advance\n";
        return 1;
    }

    std::cout << "T47-IoVecWriteState PASS\n";
    return 0;
}
