...
int main() {
    ...
    if (!scheduler) {
        std::cerr << "[T44] missing IO scheduler\n";
        server.stop();
        return 1;
    }
    ...
    if (!g_done.load(std::memory_order_acquire)) {
        std::cerr << "[T44] client timed out at stage "
                  << static_cast<int>(g_stage.load(std::memory_order_acquire)) << "\n";
        return 1;
    }
    if (!g_ok.load(std::memory_order_acquire)) {
        std::cerr << "[T44] client shutdown did not complete successfully, stage "
                  << static_cast<int>(g_stage.load(std::memory_order_acquire)) << "\n";
        return 1;
    }

    std::cout << "T44-H2cClientShutdown PASS\n";
    return 0;
}
