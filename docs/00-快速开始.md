...
- `GALAY_HTTP_DISABLE_FRAMEWORK_LOG`：是否在编译期裁剪框架日志（WS/HTTP/HTTPS/WSS/H2C/H2）
...
Coroutine handleWebSocket(WsConn& ws_conn) {
    ...
    // 发送欢迎消息
    auto welcome = co_await writer.sendText("Welcome!");
    if (!welcome) {
        co_return;
    }
    ...
    while (true) {
        ...
        if (opcode == WsOpcode::Text) {
            auto send_result = co_await writer.sendText("Echo: " + message);
            if (!send_result) {
                break;
            }
        }
