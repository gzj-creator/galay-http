...
// GET /users/:id
// 路由参数由 HttpRouter 自动注入到 HttpRequest 中
Coroutine getUserHandler(HttpConn& conn, HttpRequest req) {
    // 通过 getRouteParam 获取路由参数
    std::string id = req.getRouteParam("id");

    auto lock_result = co_await db.mutex.lock();
    if (!lock_result) {
        co_return;
    }

    auto it = db.users.find(id);
    bool found = (it != db.users.end());
    std::string user_name = found ? it->second : "";
    db.mutex.unlock();

    HttpResponse response;
    if (found) {
        response = Http1_1ResponseBuilder::ok()
            .json("{\"id\":\"" + id + "\",\"name\":\"" + user_name + "\"}")
            .build();
    } else {
...
int main(int argc, char* argv[]) {
    ...
    StaticFileConfig file_config;
    file_config.mode = StaticFileConfig::TransferMode::AUTO;
    router.mount("/", root_dir, file_config);
...
Coroutine handleWebSocket(WsConn& ws_conn) {
    ...
    auto welcome_result = co_await writer.sendText("Welcome to WebSocket server!");
    if (!welcome_result) {
        co_return;
    }
...
Coroutine wsClientExample() {
    ...
    auto close_frame_result = co_await writer.sendClose(1000, "Normal closure");
    if (!close_frame_result) {
        std::cerr << "发送关闭帧失败\n";
    }
...
- `examples/include/E9-H2cEchoServer.cpp` - H2c 服务器（HTTP/2 over cleartext）
- `examples/include/E10-H2cEchoClient.cpp` - H2c 客户端（HTTP/2 over cleartext）
- `examples/include/E11-StaticServer.cpp` - 静态文件服务器
- `examples/include/E12-HttpProxy.cpp` - HTTP 反向代理
- `examples/include/E13-H2EchoServer.cpp` - H2 服务器（HTTP/2 over TLS）
- `examples/include/E14-H2EchoClient.cpp` - H2 客户端（HTTP/2 over TLS）
