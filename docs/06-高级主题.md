...
- 在 `Coroutine` 中禁止使用阻塞同步原语（线程锁、条件变量等）。
...
```cpp
Coroutine loggingHandler(HttpConn& conn, HttpRequest req) {
...
```cpp
Coroutine authGate(HttpConn& conn, HttpRequest req) {
...
struct BroadcastManager {
    ...
    Coroutine addClient(WsWriter* writer) {
...
struct RateLimiter {
    ...
    Coroutine allowRequest(const std::string& client_ip, bool& allowed) {
        auto lock_result = co_await mutex.lock();
        if (!lock_result) {
            allowed = false;
            co_return;
