...
```bash
cmake -S . -B build \
  -DCMAKE_PREFIX_PATH="/path/to/galay-kernel/install;/path/to/spdlog/install"
```
...
```cmake
find_package(GalayHttp CONFIG REQUIRED)
target_link_libraries(your_target PRIVATE GalayHttp::galay-http)
