...
## 最新补充（2026-03-01，WebSocket 写路径继续优化）

- 变更：`WsWriter` 非 io_uring 路径去掉 `syncIovecs` 拷贝，直接使用 writer 内部 iovec 写出
- 说明：同机本地回环，`100 clients / 8s / 1024B`
- 原始日志：
  - `/tmp/ws_perf_after_iovdirect_20260301_232430`
  - `/tmp/ws_perf_after_iovdirect_rerun_20260301_232525`

| 组别 | 轮次1 | 轮次2 |
|---|---|---|
| B5/B6（default） | `119328 msg/s` | `118553 msg/s` |
| B5/B6（nolog） | `176541 msg/s` | `178452 msg/s` |
| B5/B6（nolog 吞吐） | `172.403 MB/s` | `174.27 MB/s` |

## 最新补充（2026-03-01，HTTP/2 顺序优化后）

- 构建：`Release`（`build-ssl`）
- 说明：同机本地回环，压测前等待 10~20 秒可显著降低波动

### 压测日志注意事项

- 建议开启编译期日志裁剪：`-DGALAY_HTTP_DISABLE_FRAMEWORK_LOG=ON`
- `B5/B6` 默认关闭日志；如需排查可设置：`GALAY_HTTP_BENCH_LOG=1`

### 关键场景与参数

- B10/B11（H2c Mux）：
  - 服务端：`./build-ssl/benchmark/B10-H2cMuxServer 9080 4`
  - 客户端：`./build-ssl/benchmark/B11-H2cMuxClient 127.0.0.1 9080 120 120 500 240 4`
- B12/B13（H2 TLS）：
  - 服务端：`./build-ssl/benchmark/B12-H2Server 9443 4 cert/test.crt cert/test.key`
  - 客户端：`./build-ssl/benchmark/B13-H2Client 127.0.0.1 9443 400 5000 16 180 4`

### 实测结果

| 组别 | 轮次1 | 轮次2 | 说明 |
|---|---|---|---|
| B10/B11（H2c Mux，7,200,000 req） | `215549 req/s` | - | 长压单轮 |
| B12/B13（H2 TLS，2,000,000 req） | `155388 req/s` | `154083 req/s` | 同参数双轮 |

### 备注

- 本轮优化后，`B13-H2Client` 参数已固定为：`[host] [port] [clients] [requests] [streams_per_batch] [max_wait] [io_threads]`。
- TLS 场景实测主瓶颈仍在 `SSL_write/tls13_cipher`，业务层优化应优先减少无效分配与频繁小包写入。
